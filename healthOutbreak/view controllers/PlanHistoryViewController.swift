                    ...
                    if let jsonData = json[result_resp].description.data(using: .utf8) {
                        do {
                            ...
                            for getTravelData in arrGetTravel {
                                let dateStartDate = getTravelData.startDate.dateFromString("dd/MM/yyyy")
                                
                                let diffInDays = Calendar.current.dateComponents([.day], from:Date(), to:dateStartDate).day
                                if diffInDays! < 0 {
                                    historyGetTravel.append(getTravelData)
                                }
                            }
...
extension PlanHistoryViewController:UITableViewDelegate,UITableViewDataSource {
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return 130
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return historyGetTravel.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier:"cell", for: indexPath) as! PlanListTableViewCell
        
        let data = historyGetTravel[indexPath.row]
